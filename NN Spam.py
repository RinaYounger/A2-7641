...
from func.nn.activation import RELU, LinearActivationFunction
...
from shared import SumOfSquaresError, DataSet, Instance
from opt.example import NeuralNetworkOptimizationProblem
from opt.example import NeuralNetworkEvaluationFunction
...
def main():
    ...
    oa_names = ["RHC-Bam act", "SA-Bam act", "GA-Bam act"]
    results = ""

    for name in oa_names:
        factory = BackPropagationNetworkFactory()
        classification_network = factory.createClassificationNetwork([INPUT_LAYER, HIDDEN_LAYER, OUTPUT_LAYER], LogisticSigmoid())
        networks.append(classification_network)
        nnop.append(NeuralNetworkOptimizationProblem(data_set, classification_network, measure))

    oa.append(RandomizedHillClimbing(nnop[0]))
    oa.append(SimulatedAnnealing(1E9, .3, nnop[1]))
    # oa.append(StandardGeneticAlgorithm(100, 50, 10, nnop[2]))

    # factory = BackPropagationNetworkFactory()
    # network = factory.createClassificationNetwork([INPUT_LAYER, HIDDEN_LAYER, OUTPUT_LAYER], LinearActivationFunction())
    # networks.append(network)
    # oa.append( BatchBackPropagationTrainer(DataSet(training), network, SumOfSquaresError(), RPROPUpdateRule()))
